...
const addReview = async (req, res) => {
  try {
    ...
    if (!ground || !ground.company || !ground.company._id) {
      return res
        .status(400)
        .json({ message: "Company not found for this booking's ground." });
    }
...
const getCompanyReviews = async (req, res) => {
  try {
    ...
    const reviews = await Review.find({ "company._id": companyId }).sort({
      createdAt: -1,
    });
...
// ✅ Get all reviews (for admin or dashboard use)
...
module.exports = { addReview, getCompanyReviews, getAllReviews };
